...
async def main():
    ...
    async with schwabdev.ClientAsync(
        ...
        linked_accounts = await client.linked_accounts()
        ...
        print(await client.account_details_all())
        await pause()

        print("\nGet specific account positions (uses default account, can be changed)")
        print(await client.account_details(account_hash, fields="positions"))
        await pause()

        print("\nGet orders for a linked account")
        now = datetime.datetime.now(datetime.timezone.utc)
        thirty_days_ago = now - datetime.timedelta(days=30)
        print(await client.account_orders(account_hash, thirty_days_ago, now))
        await pause()
        ...
        print(await client.order_details(account_hash, order_id))
        ...
        print(await client.preferences())
        await pause()

        print("\nGet a list of quotes")
        print(await client.quotes(["AAPL", "AMD"]))
        await pause()
        ...
        print(await client.option_expiration_chain("AAPL"))
        await pause()

        print("\nGet price history for a symbol")
        print(await client.price_history("AAPL", "year"))
        await pause()

        print("\nGet movers for an index")
        print(await client.movers("$DJI"))
        await pause()

        print("\nGet marketHours for symbols")
        print(await client.market_hours(["equity", "option"]))
        # print(await client.market_hours(["equity", "option"])  # already using list form
        await pause()

        print("\nGet marketHours for a market")
        print(await client.market_hour("equity"))
        await pause()

        print("\nGet instruments for a symbol")
        print(await client.instruments("AAPL", "fundamental"))
        await pause()

        print("\nGet instruments for a cusip")
        print(await client.instrument_cusip("037833100"))  # 037833100 = AAPL
        await pause()
