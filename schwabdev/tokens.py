...
class Tokens:
    def __init__(
        ...
        self._refresh_token_timeout = (
            7 * 24 * 60 * 60
        )  # in seconds (7 days from schwab)
        self._logger = logger  # logger
        self._call_for_auth = call_for_auth  # function to call for custom auth
        self._cipher_suite = (
            Fernet(encryption) if (encryption and len(encryption) > 16) else None
        )  # encryption suite for tokens
        ...
        if loaded:
            ...
            self._logger.warning(
                "[Schwabdev] Could not load tokens from DB, starting authorization flow."
            )
    ...
    def _dec(self, s: str) -> str:
        if not s:
            ...
            return s
        elif not self._cipher_suite and s.startswith(
            _ENC_PREFIX
        ):  # no cipher but encrypted
    ...
    def _load_tokens_from_db(self) -> bool:
        ...
        try:
            ...
            self._logger.error(
                f"[Schwabdev] Could not decrypt tokens from sqlite database ({e})"
            )
            return False
    ...
    def update_tokens(self, force_access_token=False, force_refresh_token=False):
        ...
        if (rt_delta < refresh_threshold) or force_refresh_token:
            self._logger.warning(
                f"The refresh token {'has expired!' if rt_delta < datetime.timedelta(0) else 'is expiring soon (<60min)!'}"
            )
    ...
    def _update_access_token(self, overwrite: bool = False):
        ...
        with self._update_lock:  # across threads
            ...
            try:
                self._cur.execute(
                    "BEGIN EXCLUSIVE"
                )  # begin early and hold throughout all db/http to limit to one new access token across instances.
            except sqlite3.Error as e:
                self._logger.error(
                    f"[Schwabdev] Could not begin exclusive transaction ({e})"
                )
                return
            self._load_tokens_from_db()
            if self._access_token_issued > last_known_at_issued and not overwrite:
                self._logger.info(
                    f"Access token updated elsewhere at {self._access_token_issued}."
                )
            ...
            try:
                try:
                    ...
                    self._logger.error(
                        f"[Schwabdev] Could not update access token (network error: {e})"
                    )
                    self._conn.rollback()  # release lock, no write performed
                    return
                if response.ok:
                    committed = self._set_tokens(
                        now, self._refresh_token_issued, response.json()
                    )
                    self._logger.info(
                        f"Access token updated at {self._access_token_issued}"
                    )
                else:
                    self._logger.error(
                        f"Could not get new access token; refresh_token likely invalid. ({response.text})"
                    )
                    self._conn.rollback()  # release lock, no write performed
                    return
            except Exception as e:
    ...
    def _update_refresh_token(self, overwrite: bool = False):
        ...
        with self._update_lock:
            ...
            try:
                self._cur.execute(
                    "BEGIN EXCLUSIVE"
                )  # so other instances know we're updating
            except sqlite3.Error as e:
                now = datetime.datetime.now(datetime.timezone.utc)
                if (
                    last_known_rt_issued < now
                ):  # refresh token is invalid, is access token?
                    if (
                        self._access_token_issued < now
                    ):  # refresh and access tokens are invalid.
                        self._logger.critical(
                            f"Refresh token and Access token are invalid, couldn't get db lock ({e})."
                        )
                        return
                        # raise Exception("Refresh token and Access token are invalid, couldn't get db lock, cannot continue.")
                    else:
                        self._logger.warning(
                            "Access token valid, Refresh token invalid"
                        )
                        return
                else:
            ...
            if self._refresh_token_issued > last_known_rt_issued and not overwrite:
                self._logger.info(
                    f"Refresh token updated elsewhere at {self._refresh_token_issued}."
                )
                self._conn.rollback()  # release exclusive
                return
            ...
            if self._call_for_auth is not None and callable(self._call_for_auth):
                ...
                try:
                    ...
                    self._logger.error(e)
                    self._logger.warning(
                        "Could not open browser for authorization (open the link manually)"
                    )

                # parse the callback url

                auth_callback = input(
                    "[Schwabdev] After authorizing, paste the address bar url here: "
                )
